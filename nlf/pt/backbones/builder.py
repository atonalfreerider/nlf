...
import nlf.pt.backbones.dinov2.hub.backbones as dinov2_backbones

import nlf.pt.backbones.efficientnet as effnet


def build_backbone():
    build_fn = get_build_fn()
    normalizer = get_normalizer()
    backbone, mean, std, out_channels = build_fn(normalizer)

    if FLAGS.lora_rank > 0:
        florch.layers.lora.apply_lora(backbone, lora_r=FLAGS.lora_rank)
...
def build_effnetv2(bn):
    effnet_size = FLAGS.backbone.rpartition('-')[2]
...
class DinoV2(nn.Module):
    def __init__(self):
        super().__init__()
        self.model = getattr(dinov2_backbones, FLAGS.backbone.replace('-', '_'))()
        self.num_features = self.model.num_features
        self.feat_side = FLAGS.proc_side // self.model.patch_size
...
class PreprocLayer(torch.nn.Module):
    def __init__(self, mean, std):
        super().__init__()
        self.mean = nn.Buffer(to_tensor(mean), persistent=False)
        self.std = nn.Buffer(to_tensor(std), persistent=False)
