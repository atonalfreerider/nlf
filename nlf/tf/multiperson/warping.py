...
def warp_images_with_pyramid(
    ...
    for i_crop in tf.range(n_crops):
        ...
        crop = warp_single_image(
            image_level[image_ids[i_crop]],
            intrinsic_matrix_level[i_crop],
            new_invprojmat[i_crop],
            distortion_coeffs[i_crop],
            output_shape,
        )
...
def warp_images(
    ...
    for i_crop in tf.range(n_crops):
        tf.autograph.experimental.set_loop_options(parallel_iterations=1000)
        crop = warp_single_image(
            images[image_ids[i_crop]],
            intrinsic_matrix[i_crop],
            new_invprojmat[i_crop],
            distortion_coeffs[i_crop],
            output_shape,
        )
...
def warp_single_image(image, intrinsic_matrix, new_invprojmat, distortion_coeffs, output_shape):
    ...
    if tf.reduce_all(distortion_coeffs == 0):
        ...
        new_coords = tf.cast(
            tf.stack(tf.meshgrid(tf.range(output_shape[1]), tf.range(output_shape[0])), axis=-1),
            tf.float32,
        )
...
def distort_points_fisheye(undist_points2d, distortion_coeffs):
    ...
    broadcast_shape = (
        ...
        + [1] * (undist_points2d.shape.rank - d.shape.rank)
        + [4]
    )
...
def undistort_points_fisheye(dist_points2d, distortion_coeffs):
    d = distortion_coeffs
    broadcast_shape = (
        ([-1] if d.shape.rank > 1 else []) + [1] * (dist_points2d.shape.rank - d.shape.rank) + [4]
    )
    d = tf.reshape(d, broadcast_shape)
...
def distortion_formula_parts_simple(undist_points2d, distortion_coeffs):
    distortion_coeffs_broadcast_shape = (
        ...
        + [1] * (undist_points2d.shape.rank - distortion_coeffs.shape.rank)
        + [5]
    )
...
def distortion_formula_parts(undist_points2d, distortion_coeffs):
    ...
    # rxy = ry @ rx
    # scale_trans = tf.stack([
    #     tf.stack([rxy[2, 2], 0, -rxy[0, 2]], axis=-1),
    #     tf.stack([0, rxy[2, 2], -rxy[1, 2]], axis=-1),
    #     tf.stack([0, 0, 1], axis=-1)], axis=-2)
    # tilt = scale_trans @ rxy
    # undistorting it is easy since this tilt matrix is invertible.
    # it's like having a more general intrinsic matrix. the scale_trans matrix
    # is there to bring the principal point to the origin, so that whatever
    # these distortion parameters may be, the principal point remains where the intrinsic matrix
    # says it is. (principal point, as in the forward looking ray that points to the Z
    # axis of the camera extrinsic rotation matrix)
    # Similarly, the scaling ensures that the effective focal length isn't changed by these
...
def corner_aligned_scale_mat(factor):
    shift = (factor - 1) / 2
    return tf.convert_to_tensor([[factor, 0, shift], [0, factor, shift], [0, 0, 1]], tf.float32)
