...
def load_dense(ex, joint_info, full_joint_info, learning_phase, rng):
    ...
    interp_weights_random = sps.vstack(
        [sps.csr_matrix((len(true_points_both), interp_weights_random.shape[1])),
         interp_weights_random])
    ...
    return dict(
        dense=dict(
            ...
            interp_weights=sparse_matrix_to_dict(interp_weights_random),  # random
        ))
