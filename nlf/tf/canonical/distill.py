...
def main():
    ...
    data_train = (
        tf.data.Dataset.from_tensor_slices(
            dict(
                embedding_true=y_train[:, :out_dim].astype(np.float32),
                query=x_train.astype(np.float32),
            )
        )
        .shuffle(reshuffle_each_iteration=True, buffer_size=len(x_train))
        .batch(batch_size)
        .repeat()
    )
    data_val = (
        tf.data.Dataset.from_tensor_slices(
            dict(
                embedding_true=y_val[:, :out_dim].astype(np.float32),
                query=x_val.astype(np.float32),
            )
        )
        .batch(batch_size)
        .repeat()
    ...
    trainer.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=WarmupCosineDecay(1e-2, 0, n_steps, 0))
    )
    ...
    # SAVE
    model.save(f'{PROJDIR}/lbo_mlp_{pos_enc_dim}fourier_{hidden_dim}gelu_{out_dim}__')
    ...
    def __call__(self, step):
        ...
        return (
            ...
            * (
                ...
                + tf.cos(
                    np.pi
                    * (step - self.warmup_steps - self.hold)
